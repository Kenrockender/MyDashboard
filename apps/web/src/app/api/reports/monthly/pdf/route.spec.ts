...
describe('GET /api/reports/monthly/pdf', () => {
  ...
  it('returns a PDF response wired to the monthly report data', async () => {
    ...
    const req = new NextRequest('http://localhost/api/reports/monthly/pdf?month=2026-01');
    ...
    expect(mockPdfService.renderReport).toHaveBeenCalledWith(
      'monthly',
      [],
      expect.objectContaining({ contextLabel: '2026-01' }),
    );
  });

...
export async function GET(req: NextRequest) {
  return withRoute(req, async () => {
    ...
    const rows = await reportsService.getExpenseBreakdown(user.userId);
    const buffer = await pdfService.renderReport('expenses', rows, { generatedAt: new Date() });
    return new Response(new Uint8Array(buffer), {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': 'attachment; filename="expense-breakdown.pdf"',
      },
    });
  });
}

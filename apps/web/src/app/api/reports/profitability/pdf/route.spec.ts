...
describe('GET /api/reports/profitability/pdf', () => {
  beforeEach(() => {
    jest.resetAllMocks();
  });

  it('returns a PDF response wired to the profitability data', async () => {
    mockRequireUser.mockResolvedValue({ userId: 'user_1' });
    mockReportsService.getProfitability.mockResolvedValue([]);
    mockPdfService.renderReport.mockResolvedValue(Buffer.from('fake-pdf'));

    const req = new NextRequest('http://localhost/api/reports/profitability/pdf');
    const res = await GET(req);

    expect(res.status).toBe(200);
    expect(res.headers.get('Content-Type')).toBe('application/pdf');
    expect(mockReportsService.getProfitability).toHaveBeenCalledWith('user_1');
    expect(mockPdfService.renderReport).toHaveBeenCalledWith('profitability', [], expect.any(Object));
  });
  ...
  it('rejects an unauthenticated request with 401', async () => {
    ...
    expect(mockReportsService.getProfitability).not.toHaveBeenCalled();
  });
});

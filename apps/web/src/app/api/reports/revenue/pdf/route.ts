...
export async function GET(req: NextRequest) {
  return withRoute(req, async () => {
    ...
    const rows = await reportsService.getRevenueBreakdown(user.userId);
    const buffer = await pdfService.renderReport('revenue', rows, { generatedAt: new Date() });
    return new Response(new Uint8Array(buffer), {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': 'attachment; filename="revenue-by-client.pdf"',
      },
    });
  });
}

...
describe('GET /api/reports/trend/pdf', () => {
  beforeEach(() => {
    jest.resetAllMocks();
  });

  it('returns a PDF response wired to the trend data', async () => {
    mockRequireUser.mockResolvedValue({ userId: 'user_1' });
    mockReportsService.getMonthlyTrend.mockResolvedValue([]);
    mockPdfService.renderTrend.mockResolvedValue(Buffer.from('fake-pdf'));

    const req = new NextRequest('http://localhost/api/reports/trend/pdf');
    const res = await GET(req);

    expect(res.status).toBe(200);
    expect(res.headers.get('Content-Type')).toBe('application/pdf');
    expect(mockReportsService.getMonthlyTrend).toHaveBeenCalledWith('user_1');
    expect(mockPdfService.renderTrend).toHaveBeenCalled();
  });

...
export class InvoicesService {
  ...
  private get collection() {
    return this.firebase.db.collection(COLLECTIONS.invoices);
  }
  ...
  async create(
    ...
    const projectDoc = await this.firebase.db
      .collection(COLLECTIONS.projects)
      .doc(projectId)
      .get();
    ...
    const incomeDocs = await Promise.all(
      dto.incomeIds.map((id) =>
        this.firebase.db.collection(COLLECTIONS.income).doc(id).get(),
      ),
    );
  ...
  async getPdfData(
    ...
    const [projectDoc, incomeDocs] = await Promise.all([
      this.firebase.db.collection(COLLECTIONS.projects).doc(invoice.projectId).get(),
      Promise.all(
        invoice.incomeIds.map((incomeId) =>
          this.firebase.db.collection(COLLECTIONS.income).doc(incomeId).get(),
        ),
      ),
    ]);
    ...
    const clientDoc = invoice.clientId
      ? await this.firebase.db.collection(COLLECTIONS.clients).doc(invoice.clientId).get()
      : null;

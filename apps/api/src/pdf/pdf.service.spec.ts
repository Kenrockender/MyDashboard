...
describe('PdfService', () => {
  ...
  it('renders an invoice to a PDF buffer', async () => {
    const buffer = await service.renderInvoice(
      {
        id: 'inv_1',
        userId: 'user_1',
        projectId: 'proj_1',
  ...
  it('renders a report to a PDF buffer, one section per currency', async () => {
    const buffer = await service.renderReport(
      'expenses',
      [
  ...
  it('renders the monthly trend to a PDF buffer, one chart per currency', async () => {
    const buffer = await service.renderTrend(
      [
  ...
  it('renders an empty trend without throwing', async () => {
    const buffer = await service.renderTrend([], { generatedAt: new Date() });
    expect(buffer.subarray(0, 5).toString('utf8')).toBe('%PDF-');
  });

  it('renders an empty report without throwing', async () => {
    const buffer = await service.renderReport('revenue', [], {

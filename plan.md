...
> **Status: Phases 0-6 below are done and shipped** — the app is built, tested, and deployed (see checkbox state throughout, and `06-Development-Roadmap.md`). **The stack diverged from the plan as written**: Phase 0/Task 0.2 through Task 1.3's code snippets describe a **Prisma + PostgreSQL (Neon)** setup that was later replaced with **Firestore via the Firebase Admin SDK**, and Task 0.4/Phase 5's `Clerk` auth was replaced with **Firebase Auth (Google sign-in)**. Those snippets are left in place below as a historical record of the original plan, not as accurate documentation of what's in the repo — for the real implementation, read `apps/web/src/server/firebase.ts`, `apps/web/src/server/require-user.ts`, and `apps/web/src/lib/firebase.ts`/`auth-context.tsx` directly, and see `02-System-Architecture.md` / `03-Database-Design.md` / `04-API-Specification.md`, which have all been updated to match the actual build.
>
> **Later still, the two-app split described throughout this plan (and below in "Architecture") was collapsed**: the NestJS API (`apps/api`) was merged into the Next.js app (`apps/web`) as Route Handlers under `apps/web/src/app/api/`, purely for one-Vercel-project deployment simplicity — no functional change. `apps/api` no longer exists. Every `apps/api/src/...` path mentioned below is historical; the equivalent code now lives at `apps/web/src/server/...`. See `07-Deployment-and-DevOps.md` §1/§2/§5/§5a for the current (single-project) deployment shape.

**Goal:** Build the single-user MVP described in `01-Product-Requirements-Document.md` — a Next.js + NestJS app that tracks project income/expenses and shows profitability on a dashboard. *(Shipped, on Firestore rather than the PostgreSQL originally scoped below; the NestJS API was later folded into the Next.js app itself — see the status note above.)*

**Architecture:** Single Next.js app — App Router pages plus `app/api/*` Route Handlers (formerly a separate NestJS REST API, merged in), Firestore via the Firebase Admin SDK, in one npm-workspaces monorepo. Full detail in `02-System-Architecture.md`, `03-Database-Design.md`, `04-API-Specification.md`.
